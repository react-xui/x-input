...
var Input = function (_Base) {
    ...
    _createClass(Input, [{
        ...
        value: function onChangeHandle(e) {
            ...
            this.setState({ value: value }, function () {
                _this8.props.onChange && _this8.props.onChange(value);
            });
        }
    }]);
        ...
        _createClass(_class2, [{
            ...
            value: function format(value, isinit, target) {
                ...
                if (!isinit) {
                    ...
                    this.setState({ value: value }, function () {
                        if (target) {
                            var tmp = String(_this10.state.value).length - rightpos;
                            // console.log(tmp,this.state.value.length,rightpos)
                            // console.log(tmp)
                            setCaretPosition(target, tmp);
                        }
                        if (!_this10.isNaN && value != '' && value != "-") {
                            value = Number(value.replace(/\,/gi, ''));
                        } else {
                            value = value.replace(/\,/gi, '');
                        }
                        _this10.props.returnType ? value = window[_this10.props.returnType](value) : String(value);
                        if (!_this10.isNaN && value === '-') {
                            return;
                        }
                        istriggerChange && _this10.props.onChange && _this10.props.onChange(value);
                    });
                } else {
                    return value;
                }
